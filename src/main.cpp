...
double lastMoveTime = 0;  // Для контроля чувствительности
bool isPaused = false;
bool isDarkTheme = true;
float fadeAlpha = 0.0f;
bool gameStarted = false;

// Структура для хранения цветов темы
struct Theme {
    Color background;
    Color text;
    Color highlight;
...
// Определяем тёмную и светлую темы
...
bool DrawThemeButton(Font font, bool &isDarkTheme, float &themeTransition) {
    Rectangle button = {360, 520, 120, 50};
    ...
    Vector2 textSize = MeasureTextEx(font, "Theme", 30, 2);
    DrawTextEx(font, "Theme", {button.x + (button.width - textSize.x) / 2, button.y + (button.height - textSize.y) / 2}, 30, 2, BLACK);

    if (clicked) {
        isDarkTheme = !isDarkTheme;
        themeTransition = 0.0f;
    }
    return clicked;
}
...
int main() {
    ...
    int highScore = LoadHighScore();  // Загружаем рекорд
    ...
    while (!WindowShouldClose()) {
        if (!gameStarted) {
            ...
            // Обновление состояния игры, если не на паузе и игра не окончена
            if (!isPaused && !game.gameOver) {
                game.HandleInput();  // Обработка ввода пользователя

                // Перемещение блока вниз через определённые интервалы времени
                double speed = 0.5 - (game.level - 1) * 0.05;  // Ускорение с каждым уровнем
            ...
            if (game.gameOver) {
                ...
                DrawTextEx(font, "GAME OVER", {250 - textSize.x / 2, 270}, 60, 6, gameOverColor);
                ...
                if (!scoreSaved) {
                    if (game.score > highScore) {
                        ...
                        highScore = game.score;  // Обновляем рекорд для отображения
                    }
                    scoreSaved = true;
                }
            }
